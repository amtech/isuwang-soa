...
1、安装dapeng项目(isuwang-soa  1.2.2)到本地仓库。或者进入dapeng项目的dapeng-code-generator 模块执行 mvn clean install ,再进入dapeng-maven-plugin模块执行 mvn clean install ，只安装这两个也行。
...
6、用插件生成的代码如果有依赖common-library的，必须用1.2.1或以上的版本

7、使用这个插件如果有依赖dapeng 必须是1.2.1或以上的版本