...
class JavaSerializerGenerator extends CodeGenerator{
  ...
  def toStructSerializerTemplate(service: Service, struct:Struct ): Elem ={
    return {
  ...
  def toArgsMethodSerializerTemplate(service: Service,method:Method): Elem ={
    return {
      <div> package com.isuwang.soa.serializer;
        ...
        **/
            ...
            <div>public class {method.name.charAt(0).toUpper + method.name.substring(1)}_argsSerializer implements TCommonBeanSerializer{lt}{service.name}Codec.{method.name}_args{gt}<block>
            {getReadMethod(method.getRequest,service.name+"Codec.")}{getWriteMethod(method.getRequest,service.name+"Codec.")}{getValidateMethod(method.getRequest,service.name+"Codec.")}

            @Override
            public String toString({service.name}Codec.{method.name}_args bean) <block> return bean == null ? "null" : bean.toString(); </block>
  ...
  def toResultMethodSerializerTemplate(service: Service,method:Method): Elem ={
    return {
      <div> package com.isuwang.soa.serializer;
        ...
        **/
        ...
        public class {method.name.charAt(0).toUpper + method.name.substring(1)}_resultSerializer implements TCommonBeanSerializer{lt}{service.name}Codec.{method.name}_result{gt}<block>
        @Override
        public {service.name}Codec.{method.response.name} read(TProtocol iprot) throws TException<block>

          {service.name}Codec.{method.response.name} bean = new {service.name}Codec.{method.response.name}();
        ...
        {getWriteMethod(method.getResponse,service.name+"Codec.")}
        {getValidateMethod(method.getResponse,service.name+"Codec.")}

        @Override
        public String toString( {service.name}Codec.{method.name}_result bean) <block> return bean == null ? "null" : bean.toString(); </block>
      </block>
  ...
  def toMetadataArgsSerializerTemplate(service: Service): Elem ={
    return {
      <div> package com.isuwang.soa.serializer;
