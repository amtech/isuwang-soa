  ...
  def toArgsMethodSerializerTemplate(service: Service,method:Method): Elem ={
    return {
      <div> package com.isuwang.soa.scala.serializer;
        ...
        *  @generated
        **/
        class {method.name.charAt(0).toUpper + method.name.substring(1)}_argsSerializer extends TCommonBeanSerializer[{service.name}Codec.{method.name}_args]<block>
        ...
        override def toString(bean: {service.name}Codec.{method.name}_args): String = if(bean == null)  "null" else bean.toString
      </block>
      </div>
    }
  }
  def toResultMethodSerializerTemplate(service: Service,method:Method): Elem ={
    return {
      <div> package com.isuwang.soa.scala.serializer;
        ...
        *  @generated
        **/
        class {method.name.charAt(0).toUpper + method.name.substring(1)}_resultSerializer extends TCommonBeanSerializer[{service.name}Codec.{method.name}_result]<block>

        @throws[TException]
        override def read(iprot: TProtocol): {service.name}Codec.{method.name}_result = <block>
          ...
          iprot.readStructEnd
          val bean = {service.name}Codec.{method.name}_result({if(method.response.fields.get(0).dataType.kind != KIND.VOID) <div>success</div>})
          validate(bean)
        ...
        override def toString(bean: {service.name}Codec.{method.name}_result): String = if(bean == null)  "null" else bean.toString
      </block>
