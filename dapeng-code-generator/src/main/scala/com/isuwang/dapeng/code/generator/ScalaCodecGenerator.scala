...
class ScalaCodecGenerator extends CodeGenerator {
      ...
      <div>package {service.namespace.substring(0, service.namespace.lastIndexOf("."))}
        ...
        **/
        ...
        {toMethodArrayBuffer(service.methods).map{(method: Method)=> {
          ...
          <div>
            ...
            def apply(iface: {service.namespace}.{service.name}Async, args: {method.name}_args):Future[{method.name}_result] = <block>
              ...
              _result.thenApply({if(method.response.fields.get(0).dataType.kind != KIND.VOID) "Void" else "key"} =><block>
                  val res = new {service.name}AsyncCodec.{method.name}();
              </block>).get()
        ...
        "getServiceMetadata", new GetServiceMetadata_argsSerializer(), new GetServiceMetadata_resultSerializer()) <block>
          ...
          @throws[TException]
          override def apply(iface: {service.namespace}.{service.name}Async, args: getServiceMetadata_args): getServiceMetadata_result = <block>

            val source = scala.io.Source.fromInputStream({service.name}AsyncCodec.getClass.getClassLoader.getResourceAsStream("{oriNamespace}.{service.name}.xml"))
            val success = source.mkString
            source.close
            getServiceMetadata_result(success)
          </block>
  ...
  def toScalaWriteElement(dataType: DataType, index: Int):Elem = {
    ...
    dataType.kind match {
      ...
      case KIND.STRUCT => <div> new {if(dataType.qualifiedName.contains("com.isuwang.soa.scala")) dataType.qualifiedName.substring(0,dataType.qualifiedName.lastIndexOf("."))+".serializer."+dataType.qualifiedName.substring(dataType.qualifiedName.lastIndexOf(".")+1) else dataType.qualifiedName.substring(0,dataType.qualifiedName.lastIndexOf(".")).replace("com.isuwang.soa","com.isuwang.soa.scala")+".serializer."+dataType.qualifiedName.substring(dataType.qualifiedName.lastIndexOf(".")+1)}Serializer().write(elem{index}, oprot)</div>
      case KIND.LIST => <div>
