...
class ScalaCodecGenerator extends CodeGenerator {
      ...
      <div>package {service.namespace.substring(0, service.namespace.lastIndexOf("."))}
        ...
        **/
        ...
        toStructArrayBuffer(service.structDefinitions).map{(struct:Struct)=>{
          <div>class {struct.name}Serializer implements TScalaBeanSerializer[{struct.getNamespace() + "." + struct.name}]<block>
            {getReadMethod(struct)}{getWriteMethod(struct)}{getValidateMethod(struct)}

            @throws[TException]
            override def toString(bean: com.isuwang.dapeng.demo.domain.Order): String = <block>
  ...
  def getReadMethod(struct: Struct): Elem = {
    <div>
      @throws[TException]
      override def read(TProtocol iprot): {toStructName(struct)} = <block>
